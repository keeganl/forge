...
//    }
    ...
    // ------------------------------------
    Shader meshShader("../external/shaders/vert.glsl", "../external/shaders/frag.glsl");
    Shader lightShader("../external/shaders/light/vert.glsl", "../external/shaders/light/frag.glsl");
    Shader screenShader("../external/shaders/framebuffer/vert.glsl", "../external/shaders/framebuffer/frag.glsl");
    Shader skyboxShader("../external/shaders/skybox/vert.glsl", "../external/shaders/skybox/frag.glsl");
    ...
    glm::vec3 front;
    EditorCamera camera = EditorCamera();
...
//        }
        ...
        for (int i = 0; i < scenes.size(); i++) {
            ...
            scenes[i]->getModelMatrix() = glm::translate(glm::mat4(1.0f), scenes[i]->getPosition());

            scenes[i]->getModelMatrix() =  scenes[i]->getModelMatrix() * (
                    glm::rotate(glm::mat4(1.0f), glm::radians(scenes[i]->getRotation().x), glm::vec3(1.0,0.0f,0.0f)) *
                    glm::rotate(glm::mat4(1.0f), glm::radians(scenes[i]->getRotation().y), glm::vec3(0.0,1.0f,0.0f)) *
                    glm::rotate(glm::mat4(1.0f), glm::radians(scenes[i]->getRotation().z), glm::vec3(1.0,0.0f,1.0f))
            );



            if (scenes[i]->getMeshes()[0].textures.empty()) {
                scenes[i]->setMixVal(0.0f);
                meshShader.set1DFloat("mixVal", scenes[i]->getMixVal());
            } else {
                scenes[i]->setMixVal(1.0f);
                meshShader.set1DFloat("mixVal", scenes[i]->getMixVal());
            }
            ...
            if (!checkLights(scenes)) {
                ...
                if (scenes[i]->getType() == LIGHT) {
                    meshShader.use();
                    meshShader.set3DFloat("lightPos", scenes[i]->getPosition()[0], scenes[i]->getPosition()[1], scenes[i]->getPosition()[2]);
                    meshShader.set3DFloat("lightColor", scenes[i]->getColor()[0], scenes[i]->getColor()[1], scenes[i]->getColor()[2]);
                    meshShader.set3DFloat("viewPos", camera.pos.x, camera.pos.y, camera.pos.z);

                    lightShader.use();
                    lightShader.setMat4("model", scenes[i]->getModelMatrix());
                    lightShader.setMat4("view", view);
                    lightShader.setMat4("projection", projection);
                    lightShader.set3DFloat("lightColor", scenes[i]->getColor()[0], scenes[i]->getColor()[1], scenes[i]->getColor()[2]);

                    scenes[i]->Draw(lightShader);
                }
                else if (scenes[i]->getType() == MODEL){
                    meshShader.use();
                    scenes[i]->getModelMatrix() = glm::scale(scenes[i]->getModelMatrix(), scenes[i]->getScale());

                    meshShader.setMat4("model", scenes[i]->getModelMatrix());
                    meshShader.setMat4("projection", projection);
                    meshShader.setMat4("view", view);
                    meshShader.set3DFloat("scaleAxes", scenes[i]->getScale()[0], scenes[i]->getScale()[1], scenes[i]->getScale()[2]);
                    meshShader.set3DFloat("objectColor", scenes[i]->getColor()[0], scenes[i]->getColor()[1], scenes[i]->getColor()[2]);
//                    meshShader.set1DFloat("scale", scenes[i]->uniformScale);
        ...
        if (showSettingsWindow)         { GuiLayer::showSettings(&showSettingsWindow); }
        ...
        // menu bar
        GuiLayer::drawMenubar(openFilePopup, openSavePopup, openSettingsPopup);
...
//            ImGui::Checkbox("Set MSAA", &useMultiSampling);
        ...
        ImGui::Begin("Scene Objects");
        {
            for (int i = 0; i < scenes.size(); i++) {
                ...
                ImGui::Selectable(scenes[i]->getName().c_str(), scenes[i]->getSelected());
                if (ImGui::IsItemClicked()) {
                    std::cout << scenes[i]->getName().c_str() << std::endl;
                    scenes[i]->setSelected();
                }
            }
        ...
        if (scenes.size() > 0)
        {
            ImGui::Begin("Mesh Properties");
            {
                ...
                for (auto &m: scenes) {
                    if (m->getSelected()) {
                        bool tNode = ImGui::TreeNodeEx((void*)"###", ImGuiTreeNodeFlags_DefaultOpen, "%s", m->getName().c_str());
                        if (tNode) {
                            ImGui::PushID(i);
                            ImGui::AlignTextToFramePadding();
                            ImGui::Text("Name");
                            ImGui::SameLine();
                            ImGui::InputText("", m->getName().data(), m->getName().size()+1);
                            ImGui::PopID();
...
//                                ImGui::PopID();
                                ...
                                ImGui::PushID(&m->getScale()[0]);
                                ImGui::AlignTextToFramePadding();
                                ImGui::Text("Axis Scale");
                                ImGui::SameLine();
                                ImGui::SliderFloat3("\t", &m->getScale()[0], 0.0, 100.0);
                                ImGui::PopID();

                                ImGui::PushID(&m->getModelMatrix());
                                ImGui::AlignTextToFramePadding();
                                ImGui::Text("Model Position");
                                ImGui::SameLine();
                                ImGui::SliderFloat3("\t", &m->getPosition()[0], -nearClipping, farClipping);
                                ImGui::PopID();

                                ImGui::PushID(&m->getRotation()[0]);
                                ImGui::AlignTextToFramePadding();
                                ImGui::Text("Model Rotation");
                                ImGui::SameLine();
                                ImGui::SliderFloat3("\t", &m->getRotation()[0], -1000.0, 1000.0);
                                ImGui::PopID();
                            }

                            if (ImGui::CollapsingHeader("Mesh Material", ImGuiTreeNodeFlags_DefaultOpen)) {
                                ImGui::PushID(&m->getColor());
                                ImGui::Text("Color:");
                                ImGui::SameLine();
//                                GuiLayer::HelpMarker(
//                                        "Click on the color square to open a color picker.\n"
//                                        "CTRL+click on individual component to input value.\n");
                                ImGui::ColorPicker4("Mesh color", (float*)&m->getColor());
                                ImGui::PopID();

                                ImGui::PushID(&m->getMeshes()[0]);
                                for (int i = 0; i < m->getMeshes().size(); i++) {
                                    if (m->getMeshes()[i].textures.empty()) {
                                        ImGui::PushID("texture_");
                                        ImGui::Text("Add Texture");
                                        if (ImGui::ImageButton((void *) (intptr_t) default_texture, ImVec2(50, 50))) {
                                            m->getMeshes()[i] = saveTexture(textureDialog, m->getMeshes()[i]);
                                        }
                                        ImGui::PopID();
                                    }
                                    else {
                                        ...
                                        for (Texture &t : m->getMeshes()[i].textures) {
        ...
        GuiLayer::drawCameraEditor(camera);
